    ...
    on_supported_os.each do |os, os_facts|
      context "on #{os}" do
        ...
        context 'with default parameters' do
          ...
          it { is_expected.to create_file('/etc/openldap/DB_CONFIG').with_content(%r{set_data_dir}) }
          it { is_expected.to create_file('/etc/openldap/default.ldif').with_content(%r{dn: DC=host,DC=net}) }
          it { is_expected.to create_file('/etc/openldap/default.ldif').with_content(%r{pwdCheckModule: .*check_password.so}) }

          # Users
          it { is_expected.to create_file('/etc/openldap/default.ldif').with_content(%r{gidNumber: 100}) }

          # Administrators
          it { is_expected.to create_file('/etc/openldap/default.ldif').with_content(%r{gidNumber: 700}) }
          it { is_expected.to create_file('/etc/sysconfig/slapd').with_content(%r{SLAPD_URLS.*ldap.*://}) }
        ...
        context 'with pki = false' do
          ...
          it { is_expected.not_to create_file('/etc/pki/simp_apps/openldap/x509') }
          it { is_expected.to create_file('/etc/openldap/slapd.conf') }
          it { is_expected.to create_file('/etc/openldap/slapd.conf').without_content(%r{TLSCertificateFile}) }
        end
        ...
        context 'with pki = true and openldap-servers < 2.4.40' do
          ...
          it { is_expected.to create_file('/etc/openldap/slapd.conf').without_content(%r{TLSProtocolMin}) }
          it { is_expected.to create_file('/etc/openldap/slapd.conf').with_content(%r{TLSCipherSuite DEFAULT:!MEDIUM}) }
        end
        ...
        context 'with pki = simp' do
          ...
          it { is_expected.to create_pki__copy('openldap') }
          it { is_expected.to create_file('/etc/pki/simp_apps/openldap/x509') }
          it { is_expected.to create_file('/etc/openldap/slapd.conf').with_content(%r{TLSCertificateFile}) }
        end
        ...
        context 'do_not_use_iptables' do
          ...
          it { is_expected.not_to create_iptables__listen__tcp_stateful('allow_ldap') }
          it { is_expected.not_to create_iptables__listen__tcp_stateful('allow_ldaps') }
        end
        ...
        context 'audit_transactions' do
          ...
          it { is_expected.to create_rsyslog__rule__drop('1_drop_openldap_passwords').with_rule(%r{contains\s+'Password::\s+'}) }
        ...
        context 'audit_transactions_no_audit_to_syslog' do
          ...
          it { is_expected.not_to create_rsyslog__rule__drop('1_drop_openldap_passwords') }
        ...
        context 'logging_disabled' do
          ...
          it { is_expected.not_to create_rsyslog__rule__local('05_openldap_local') }
          it { is_expected.not_to create_logrotate__rule('slapd') }
        end
        ...
        context 'threads_is_dynamic' do
          ...
          it { is_expected.to create_file('/etc/openldap/slapd.conf').without_content(%r{threads   8}) }
        end
        ...
        context 'threads_is_user_overridden' do
          ...
          it { is_expected.to create_file('/etc/openldap/slapd.conf').with_content(%r{threads   20}) }
          it { is_expected.to create_file('/etc/openldap/slapd.conf').without_content(%r{threads   8}) }
          it { is_expected.to create_file('/etc/openldap/slapd.conf').without_content(%r{threads   16}) }
        end
      end
    end
  end
